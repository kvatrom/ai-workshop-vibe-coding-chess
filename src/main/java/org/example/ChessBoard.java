...
/**
 * Minimal chess board model starting from the standard initial position. Supports applying a subset
 * of SAN moves for White only (first move from the initial position): - Pawn pushes: a3/a4 .. h3/h4
 * (no captures, no promotions) - Knight moves: Na3, Nc3, Nd2, Ne2, Nf3, Nh3 (from b1 or g1) If a
 * move is legal under these constraints, returns a MoveResult containing origin/destination and the
 * board after the move. Otherwise returns illegal result.
 */
public class ChessBoard {
  ...
  /**
   * Apply a very simple subset of SAN for either side on the current board state. Supported: - Pawn
   * pushes: one or two squares forward from starting rank, no captures, no promotions. - Knight
   * non-capturing moves: N<dest>. Captures and disambiguation are not supported. Ignores trailing +
   * or #. Returns MoveResult with updated board when legal.
   */
  public MoveResult tryApplySanSimple(String sanInput, boolean whiteToMove) {
    ...
    if (san.length() == 4
        && Character.isLetter(san.charAt(0))
        && (san.charAt(1) == 'x' || san.charAt(1) == 'X')
        && Character.isLetter(san.charAt(2))
        && Character.isDigit(san.charAt(3))) {
      ...
      if (fromFile < 0
          ...
          || toRank > 7
          || fromRank < 0
          || fromRank > 7) return MoveResult.illegal();
    ...
    if (san.length() == 2
        && Character.isLetter(san.charAt(0))
        && Character.isDigit(san.charAt(1))) {
      ...
      int dir = whiteToMove ? 1 : -1;
      int startRank =
          whiteToMove ? 1 : 6; // indices: white pawns on rank 2 (1), black on rank 7 (6)
      char pawnChar = whiteToMove ? 'P' : 'p';
      ...
      if (fromRankTwo >= 0
          && fromRankTwo < 8
          && getAt(toFile, fromRankTwo) == pawnChar
          && fromRankTwo == startRank
          && getAt(toFile, midRank) == ' ') {
        ChessBoard after = copy();
  ...
  public static record MoveResult(
      boolean legal, int fromFile, int fromRank, int toFile, int toRank, ChessBoard after) {
    public static MoveResult illegal() {
      return new MoveResult(false, -1, -1, -1, -1, null);
    }

    public static MoveResult legal(
        int fromFile, int fromRank, int toFile, int toRank, ChessBoard after) {
  ...
  /**
   * Try to apply a SAN move from the initial position for White's first move only. Supported: pawn
   * pushes (one/two squares, same file, no capture), knight moves (from b1/g1).
   */
  public MoveResult tryApplySanFromInitialWhite(String sanInput) {
    ...
    if (Character.toUpperCase(san.charAt(0)) == 'N') {
      ...
      for (int[] o : origins) {
        if (knightCanMove(o[0], o[1], toFile, toRank)
            && getAt(o[0], o[1]) == 'N'
            && getAt(toFile, toRank) == ' ') {
          ChessBoard after = copy();
          after.setAt(o[0], o[1], ' ');
    ...
    if (san.length() >= 2
      ...
      // ignore trailing + or #
      if (san.length() == 3 && !(san.charAt(2) == '+' || san.charAt(2) == '#'))
        return MoveResult.illegal();
  ...
  // Generate simplified pseudo-legal moves: pawns (pushes and diagonal captures, no en-passant),
  // knights (including captures).
      ...
      for (int f = 0; f < 8; f++) {
        for (int r = 0; r < 8; r++) {
          ...
          if (p == 'P') {
            ...
            // captures
            if (toR < 8) {
              if (f - 1 >= 0 && isBlack(getAt(f - 1, toR)))
                moves.add(new SimpleMove(f, r, f - 1, toR));
              if (f + 1 < 8 && isBlack(getAt(f + 1, toR)))
                moves.add(new SimpleMove(f, r, f + 1, toR));
            }
          } else if (p == 'N') {
      ...
      for (int f = 0; f < 8; f++) {
        for (int r = 0; r < 8; r++) {
          ...
          if (p == 'p') {
            ...
            // captures
            if (toR >= 0) {
              if (f - 1 >= 0 && isWhite(getAt(f - 1, toR)))
                moves.add(new SimpleMove(f, r, f - 1, toR));
              if (f + 1 < 8 && isWhite(getAt(f + 1, toR)))
                moves.add(new SimpleMove(f, r, f + 1, toR));
            }
          } else if (p == 'n') {
                ...
                java.util.stream.IntStream.range(0, 8)
                    .map(
                        f -> {
                          ...
                          if (isWhite(p)) return v;
                          else if (isBlack(p)) return -v;
                          else return 0;
                        })
                    .sum())
        .sum();
