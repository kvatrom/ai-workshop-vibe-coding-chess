...
public class ChessMoveDescriber {
  ...
  private static final Pattern CASTLE_KINGSIDE =
      Pattern.compile("^O-O[+#]?$", Pattern.CASE_INSENSITIVE);
  private static final Pattern CASTLE_QUEENSIDE =
      Pattern.compile("^O-O-O[+#]?$", Pattern.CASE_INSENSITIVE);

  // Piece move, with optional disambiguation and capture
  // Examples: Nf3, Nbd2, R1e1, Qh5+, Bxe6, Raxb7#, Nb1xc3
  private static final Pattern PIECE_MOVE =
      Pattern.compile(
          "^([KQRBN])"
              + // 1: piece
              "([a-h1-8]?)"
              + // 2: optional disambiguation (file or rank)
              "(x)?"
              + // 3: capture flag
              "([a-h][1-8])"
              + // 4: destination square
              "(=([QRBN]))?"
              + // 5: (unused full), 6: promotion piece (rare for piece moves; mostly for pawns)
              "([+#])?$", // 7: check/mate
          Pattern.CASE_INSENSITIVE);

  // Pawn move: e4, exd5, e8=Q, exd8=Q+
  private static final Pattern PAWN_MOVE =
      Pattern.compile(
          "^([a-h])"
              + // 1: pawn file
              "(x)?"
              + // 2: capture flag (if present, next is target file)
              "([a-h])?"
              + // 3: target file when capturing (optional)
              "([1-8])"
              + // 4: target rank
              "(=([QRBN]))?"
              + // 5: (unused full), 6: promotion piece
              "([+#])?"
              + // 7: check/mate
  ...
  public static String describe(String sanInput) {
    ...
    // If nothing matched
    return "Unrecognized or unsupported move notation: '"
        + sanInput
        + "'. Supported examples: d4, Nf3, exd5, O-O, c8=Q#, Nbd2, R1e1.";
  }
